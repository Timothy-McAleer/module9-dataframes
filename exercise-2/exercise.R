...
increase <- runif(100, -10, 50) + paid


# Create a data.frame 'salaries' by combining the 3 vectors you just made
# Remember to set `stringsAsFactors=FALSE`!
salaries <- data.frame(employees, paid, increase)


# Create a column 'raise' that stores the size of the raise between 2014 and 2015
salaries$raise <- salaries$increase - salaries$paid
...
how.many <- length(salaries$got.raise)

# What was the value of the highest raise?
highest <- max(salaries$raise)

# What was the "name" of the employee who received the highest raise?
name <- salaries[salaries$raise == highest, "employees"]
